...
from nala_cli import git_toplevel
...
# verified directly against the provider. nala uses this to rebuild a
...
def default_config_path() -> Path:
    """Config resolution: NALA_CONFIG, then the project's .nala/config.json
    when inside a git repo, then the user config. CLI --config wins above all
    (callers pass it as config_path)."""
    env_path = os.environ.get("NALA_CONFIG")
    if env_path:
        return Path(env_path).expanduser()
    toplevel = git_toplevel()
    if toplevel is not None:
        project_config = toplevel / ".nala" / "config.json"
        if project_config.is_file():
            return project_config
    return Path("~/.nala/config.json").expanduser()
...
def resolve_provider(
    ...
    requested_provider = (provider or config.get("provider") or "openrouter").lower()
...
def generate_text_with_usage(
    ...
    if provider == "openrouter":
        ...
        # nala-supported block-cache API.
