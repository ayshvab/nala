...
def scan_root(root: Path) -> list[Artifact]:
    ...
    try:
        import nala_campaign_lib as campaign_lib
    except ImportError:
        campaign_lib = None
...
def regenerate_digest(root: Path, max_bytes: int = DIGEST_MAX_BYTES) -> Path | None:
    ...
    header = (
        "# Knowledge digest\n"
        "(regenerated by nala-distill; edit the category files, not this file)\n"
    )
...
def _default_summarize(path: Path, provider: str, model: str, config_path: Path | None) -> str:
    summarize_script = Path(__file__).resolve().parent.parent / "nala-file-summarize"
    command = [
    ...
    if not isinstance(summary, str) or not summary.strip():
        raise RuntimeError("nala-file-summarize returned no summary")
    return summary
...
def _summary_backfill_candidates(conversations: Path) -> list[tuple[Path, dict]]:
    ...
    the on-demand LLM upgrade nala's instant saves defer to maintenance."""
...
def _backfill_saved_summaries(conversations: Path, generate, emit) -> tuple[list[str], list[str]]:
    ...
    for index_file, entry in _summary_backfill_candidates(conversations):
        ...
        prompt = (
            "Summarize this saved nala conversation in 50 words or fewer, for a "
...
def run_gc(
    ...
    *,
    ...
    for index, artifact in enumerate(harvest_candidates, start=1):
        ...
        # A failed extraction has still consumed the source-byte budget.
