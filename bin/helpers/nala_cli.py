...
# The nala install folder: the parent of bin/ (this file lives in bin/helpers/).
INSTALL_DIR = Path(__file__).resolve().parent.parent.parent


def user_root() -> Path:
    return Path("~/.nala").expanduser()
...
def resolve_default_root(root_arg: str | None) -> Path:
    """Root resolution: --root wins; inside a git repo the project's .nala
    is the default; otherwise the user root (~/.nala)."""
    if root_arg:
        return Path(root_arg).expanduser()
    toplevel = git_toplevel()
    if toplevel is not None:
        return toplevel / ".nala"
    return user_root()
