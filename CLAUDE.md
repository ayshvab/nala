...
The same file is injected into every nala conversation via the repo's
`context.yaml` — one source of truth for both harnesses. Edit it there; do not
duplicate rules into this file.

## What this is

**nala**, a fork of [nagent](https://github.com/macton/nagent), is a small reference implementation of a data-oriented
...
`--description` (via `exit_on_description()` in `bin/helpers/nala_cli.py`).

Thin wrappers live in `bin/`; real logic lives in `bin/helpers/*_lib.py`.

- `bin/nala` (~2400 lines) — the main loop and the bulk of the system. Read this path first:
  ...
  `<nala-response>` is emitted.
- `bin/helpers/nala_llm.py` — provider abstraction. `generate_text_with_usage()`
  is the single primitive (file in → text out) for `openai`, `anthropic`, `google`,
  `cursor`. Provider churn should stay isolated here.
- `bin/nala-llm-text` / `bin/nala-llm-upload` — CLI front ends for text and
  file-upload generation.
- `bin/nala-file-edit` + `nala_file_edit_lib.py` — per-file conversations and
  git-history context.
- `bin/nala-file-split` / `-patch` / `-summarize` + their `_lib.py` — large-file
  handling (split → bounded edit → patch).

### The structured-tag protocol

The model communicates only through a fixed set of XML-ish tags (`<nala-read>`,
`<nala-file-read>`, `<nala-write>`, `<nala-shell>`, `<nala-next>`,
`<nala-conversation>`, `<nala-response>`, plus `<nala-file-patch>`). The
...
### Durable state lives under `~/.nala/`
...
- `config.json` — provider/model defaults (overridable by `NALA_CONFIG` env var
...
checked structured writes. `validate_write_path()` allows `<nala-write>` to write
...
`nala-file-edit`, not direct writes from the main loop.
...
splitters in `bin/helpers/nala-file-split-*`. Edits target segments; `nala-file-patch`
...
- Keep provider-specific code inside `nala_llm.py`.
