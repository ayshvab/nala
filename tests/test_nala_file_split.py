...
def load_split_lib():
    loader = SourceFileLoader("nala_file_split_lib", str(HELPERS / "nala_file_split_lib.py"))
    spec = importlib.util.spec_from_loader("nala_file_split_lib", loader)
...
class FileSplitCliTests(unittest.TestCase):
    ...
    def test_refresh_rebuilds_segments(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            output_dir = Path(tmp) / "split-out"
            subprocess.run(
                [str(NALA_FILE_SPLIT), "--file", str(source), "--output", str(output_dir)],
                check=True,
                capture_output=True,
                text=True,
            )
            ...
            result = subprocess.run(
                [str(NALA_FILE_SPLIT), "--refresh", str(output_dir / "index.json"), "--json"],
                capture_output=True,
                text=True,
            )
            self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def test_autodetects_js_splitter(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [str(NALA_FILE_SPLIT), "--file", str(source), "--output", str(output_dir), "--json"],
                capture_output=True,
                text=True,
            )
            self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def test_missing_file_errors(self):
        result = subprocess.run(
            [str(NALA_FILE_SPLIT), "--file", "/nonexistent/file.txt"],
            capture_output=True,
            text=True,
        )
