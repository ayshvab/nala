...
def load_file_edit_lib():
    loader = SourceFileLoader("nala_file_edit_lib", str(HELPERS / "nala_file_edit_lib.py"))
    spec = importlib.util.spec_from_loader("nala_file_edit_lib", loader)
...
class FileEditNalaTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_initial_context_includes_file_edit_rules(self):
        ...
        self.assertIn("nala-conv/briefing.md", text)  # scratch path interpolated for "conv"
        self.assertIn("nala-file-edit", text)
        self.assertIn("nala-file-read", text)

    def test_file_edit_context_allows_specific_file(self):
        target = Path("/home/macton/nala/bin/nala")
        text = self.mod.create_initial_text(
            Path("/tmp/nala-root"),
            NALA.resolve(),
            "user",
        ...
        self.assertIn(f"You may use <nala-write> on this file: {target}", text)
    ...
    def test_file_edit_context_includes_git_history_and_summary(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(self.mod, "NALA_FILE_SUMMARIZE", fake_summary):
    ...
    def test_execute_read_rejects_large_file(self):
        ...
        try:
            ...
            self.assertIn("nala-file-read", result)
        finally:
    ...
    def test_execute_file_read_inline_for_small_file(self):
        with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as handle:
            ...
            path = Path(handle.name)
        try:
            result = self.mod.execute_file_read(str(path), Path("/tmp/nala-root"))
    ...
    def test_execute_file_read_splits_large_file(self):
        ...
        try:
            ...
            self.assertIn("nala-file-patch", result)
        finally:
            path.unlink(missing_ok=True)
    ...
    def test_conversation_scratch_dir_is_per_conversation_under_tmpdir(self):
        with tempfile.TemporaryDirectory() as tmp:
            with unittest.mock.patch.dict(os.environ, {"TMPDIR": tmp}, clear=False):
                ...
                self.assertEqual(a, Path(tmp).resolve() / "nala-conv-a")
    ...
    def test_execute_write_blocks_project_path(self):
        ...
        self.assertIn("nala-file-edit", result)
...
class FileEditCliTests(unittest.TestCase):
    def test_file_edit_delegates_to_nala(self):
        loader = SourceFileLoader("nala_file_edit_mod", str(NALA_FILE_EDIT))
        spec = importlib.util.spec_from_loader("nala_file_edit_mod", loader)
        module = importlib.util.module_from_spec(spec)
        loader.exec_module(module)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            source.write_text("alpha\n", encoding="utf-8")
            fake_nala = Path(tmp) / "fake-nala"
            fake_nala.write_text("#!/usr/bin/python3\nimport sys\nprint(' '.join(sys.argv[1:]))\n", encoding="utf-8")
            fake_nala.chmod(0o755)
            module.NALA = fake_nala
    ...
    def test_file_edit_json_output(self):
        loader = SourceFileLoader("nala_file_edit_mod_json", str(NALA_FILE_EDIT))
        spec = importlib.util.spec_from_loader("nala_file_edit_mod_json", loader)
        module = importlib.util.module_from_spec(spec)
        loader.exec_module(module)
            ...
            with unittest.mock.patch.object(module.subprocess, "run") as mock_run:
                ...
                with unittest.mock.patch.object(
                    ...
                    with unittest.mock.patch.object(module.sys, "stdout", io.StringIO()) as stdout:
                        ...
                        self.assertIn("nala", payload)
                        self.assertEqual(payload["nala"]["responses"], ["done\n"])

    def test_file_edit_json_appends_multiple_files_to_index(self):
        loader = SourceFileLoader("nala_file_edit_mod_json_append", str(NALA_FILE_EDIT))
        spec = importlib.util.spec_from_loader("nala_file_edit_mod_json_append", loader)
        module = importlib.util.module_from_spec(spec)
        loader.exec_module(module)
