...
HELPERS = BIN / "helpers"
NALA = BIN / "nala"
NALA_CAMPAIGN = BIN / "nala-campaign"

sys.path.insert(0, str(HELPERS))
import nala_campaign_lib as camp


def load_nala_module():
    from importlib.machinery import SourceFileLoader

    loader = SourceFileLoader("nala_mod_campaign_tests", str(NALA))
    spec = importlib.util.spec_from_loader("nala_mod_campaign_tests", loader)
...
class ContextWiringTests(unittest.TestCase):
    ...
    def test_campaign_direction_always_present(self):
        ...
        self.assertIn("nala-campaign new", text)
    ...
    def test_campaign_item_mode_adds_contract_and_campaign_bin(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            plain = self.mod.build_initial_context(root, NALA.resolve(), "user", "conv")
...
class CampaignCliTests(unittest.TestCase):
    def test_description_flag(self):
        result = subprocess.run(
            [str(NALA_CAMPAIGN), "--description"], capture_output=True, text=True
        )
    ...
    def test_new_add_status_update_dry_run(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [str(NALA_CAMPAIGN), "--root", str(root), "new", "Demo Campaign",
                 "--goal", "do the demo"],
                capture_output=True,
                text=True,
            )
            ...
            result = subprocess.run(
                [str(NALA_CAMPAIGN), "--root", str(root), "add", "demo-campaign", "first item"],
                capture_output=True,
                text=True,
            )
            self.assertEqual(result.returncode, 0, result.stderr)
            ...
            result = subprocess.run(
                [str(NALA_CAMPAIGN), "--root", str(root), "status", "demo-campaign"],
                capture_output=True,
                text=True,
            )
            self.assertEqual(result.returncode, 0, result.stderr)
            ...
            result = subprocess.run(
                [str(NALA_CAMPAIGN), "--root", str(root), "update", "demo-campaign", "--dry-run"],
                capture_output=True,
                text=True,
            )
            self.assertEqual(result.returncode, 0, result.stderr)
