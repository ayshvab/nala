...
def load_patch_lib():
    ...
    loader = SourceFileLoader("nala_file_patch_lib", str(HELPERS / "nala_file_patch_lib.py"))
    spec = importlib.util.spec_from_loader("nala_file_patch_lib", loader)
...
class FilePatchCliTests(unittest.TestCase):
    def test_applies_segment_edits_and_updates_index(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [str(NALA_FILE_PATCH), "--index", str(index_path), "--json"],
                capture_output=True,
                text=True,
            )
    ...
    def test_dry_run_does_not_modify_source_or_index(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [str(NALA_FILE_PATCH), "--index", str(index_path), "--dry-run"],
                capture_output=True,
                text=True,
            )
            self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def test_force_ignores_hash_mismatch(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [str(NALA_FILE_PATCH), "--index", str(index_path), "--force", "--json"],
                capture_output=True,
                text=True,
            )
            self.assertEqual(result.returncode, 0, result.stderr)
