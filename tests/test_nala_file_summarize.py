...
def load_summarize_lib():
    loader = SourceFileLoader("nala_file_summarize_lib", str(HELPERS / "nala_file_summarize_lib.py"))
    spec = importlib.util.spec_from_loader("nala_file_summarize_lib", loader)
...
class FileSummarizeCliTests(unittest.TestCase):
    ...
    def test_large_file_delegates_to_split(self):
        loader = SourceFileLoader("nala_file_summarize_mod2", str(NALA_FILE_SUMMARIZE))
        spec = importlib.util.spec_from_loader("nala_file_summarize_mod2", loader)
        module = importlib.util.module_from_spec(spec)
        loader.exec_module(module)
...
class FileSplitSummarizeTests(unittest.TestCase):
    def test_split_summarize_adds_segment_summaries(self):
        loader = SourceFileLoader("nala_file_split_mod", str(NALA_FILE_SPLIT))
        spec = importlib.util.spec_from_loader("nala_file_split_mod", loader)
        module = importlib.util.module_from_spec(spec)
        loader.exec_module(module)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            fake_summarize.chmod(0o755)
            module.NALA_FILE_SUMMARIZE = fake_summarize
