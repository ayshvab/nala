...
HELPERS = BIN / "helpers"
NALA = BIN / "nala"
NALA_DISTILL = BIN / "nala-distill"

sys.path.insert(0, str(HELPERS))
import nala_distill_lib as gc
...
def load_nala_module():
    ...
    loader = SourceFileLoader("nala_mod_gc_tests", str(NALA))
    spec = importlib.util.spec_from_loader("nala_mod_gc_tests", loader)
...
class GcCliTests(unittest.TestCase):
    def test_description_flag(self):
        result = subprocess.run([str(NALA_DISTILL), "--description"], capture_output=True, text=True)
        self.assertEqual(result.returncode, 0)
        self.assertIn("Reclaim dead nala artifacts", result.stdout)
    ...
    def test_dry_run_lists_candidates_and_mutates_nothing(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [str(NALA_DISTILL), "--root", str(root)],
                capture_output=True,
                text=True,
            )
    ...
    def test_dry_run_json(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [str(NALA_DISTILL), "--root", str(root), "--json"],
                capture_output=True,
                text=True,
            )
    ...
    def test_apply_no_harvest_reclaims_without_credentials(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [str(NALA_DISTILL), "--root", str(root), "--apply", "--no-harvest"],
                capture_output=True,
                text=True,
    ...
    def test_missing_root_errors(self):
        result = subprocess.run(
            [str(NALA_DISTILL), "--root", "/nonexistent/nala-root"],
            capture_output=True,
            text=True,
        )
...
class GraduatePassTests(unittest.TestCase):
    ...
    def test_cli_merge_and_graduate_dry_runs_need_no_credentials(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [str(NALA_DISTILL), "--root", str(root), "--merge", "--graduate"],
                capture_output=True,
                text=True,
                env={"PATH": "/usr/bin:/bin"},
            )
