...
def load_nala_module():
    ...
    loader = SourceFileLoader("nala_mod_safety_tests", str(NALA))
    spec = importlib.util.spec_from_loader("nala_mod_safety_tests", loader)
...
class SafetyNetTests(unittest.TestCase):
    ...
    def test_loop_runs_safety_net_each_turn(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("<nala-turn-status ", conversation.read_text(encoding="utf-8"))
    ...
    def test_turn_status_block_carries_utc_and_token_totals(self):
        ...
        self.assertEqual(
            block,
            '<nala-turn-status utc="2026-06-12T12:00:00Z" turn="2" '
    ...
    def test_invalid_content_is_stripped_to_a_sidecar(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def fake_run(cmd, **kwargs):
                ...
                payload = {
                    "response": "<thought\nEverything done. Reporting.<nala-response>all good</nala-response>",
                    "input_tokens": 5,
                    "output_tokens": 5,
                }
                return unittest.mock.Mock(returncode=0, stdout=json.dumps(payload), stderr="")
            ...
            self.assertIn("<nala-response>all good</nala-response>", convo)
    ...
    def test_duplicate_tags_collapsed_in_conversation_without_sidecar(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def fake_run(cmd, **kwargs):
                ...
                payload = {
                    "response": "<nala-response>done</nala-response>" * 3,
                    "input_tokens": 5,
                    "output_tokens": 5,
                }
                return unittest.mock.Mock(returncode=0, stdout=json.dumps(payload), stderr="")
            ...
            convo = conversation.read_text(encoding="utf-8")
            self.assertEqual(convo.count("<nala-response>done</nala-response>"), 1)
    ...
    def test_best_of_n_direction_in_initial_context(self):
        with tempfile.TemporaryDirectory() as tmp:
            text = self.mod.build_initial_context(Path(tmp), NALA.resolve(), "user", "conv")
