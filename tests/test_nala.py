...
NALA = BIN / "nala"
NALA_LLM_TEXT = BIN / "nala-llm-text"
NALA_LLM_UPLOAD = BIN / "nala-llm-upload"
NALA_FILE_SPLIT = BIN / "nala-file-split"
NALA_FILE_PATCH = BIN / "nala-file-patch"
NALA_FILE_EDIT = BIN / "nala-file-edit"
NALA_FILE_SUMMARIZE = BIN / "nala-file-summarize"
BIN_TOOLS = (
    NALA,
    NALA_LLM_TEXT,
    NALA_LLM_UPLOAD,
...
def load_nala_module():
    from importlib.machinery import SourceFileLoader

    loader = SourceFileLoader("nala_mod", str(NALA))
    spec = importlib.util.spec_from_loader("nala_mod", loader)
    module = importlib.util.module_from_spec(spec)
    loader.exec_module(module)
    return module


def load_nala_llm_module():
    from importlib.machinery import SourceFileLoader

    loader = SourceFileLoader("nala_llm_mod", str(BIN / "helpers" / "nala_llm.py"))
    spec = importlib.util.spec_from_loader("nala_llm_mod", loader)
    module = importlib.util.module_from_spec(spec)
    loader.exec_module(module)
    return module


def load_nala_llm_text_module():
    from importlib.machinery import SourceFileLoader

    loader = SourceFileLoader("nala_llm_text_mod", str(NALA_LLM_TEXT))
    spec = importlib.util.spec_from_loader("nala_llm_text_mod", loader)
    module = importlib.util.module_from_spec(spec)
    loader.exec_module(module)
    return module


def load_nala_llm_upload_module():
    from importlib.machinery import SourceFileLoader

    loader = SourceFileLoader("nala_llm_upload_mod", str(NALA_LLM_UPLOAD))
    spec = importlib.util.spec_from_loader("nala_llm_upload_mod", loader)
    module = importlib.util.module_from_spec(spec)
    loader.exec_module(module)
    return module


def load_nala_file_summarize_module():
    from importlib.machinery import SourceFileLoader

    loader = SourceFileLoader("nala_file_summarize_mod", str(NALA_FILE_SUMMARIZE))
    spec = importlib.util.spec_from_loader("nala_file_summarize_mod", loader)
    module = importlib.util.module_from_spec(spec)
    loader.exec_module(module)
    return module


def load_nala_file_summarize_lib_module():
    from importlib.machinery import SourceFileLoader
    ...
    loader = SourceFileLoader(
        "nala_file_summarize_lib_mod",
        str(helpers / "nala_file_summarize_lib.py"),
    )
    spec = importlib.util.spec_from_loader("nala_file_summarize_lib_mod", loader)
    module = importlib.util.module_from_spec(spec)
    loader.exec_module(module)
    return module


class NalaFileSummarizeTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.mod = load_nala_file_summarize_module()
        cls.lib = load_nala_file_summarize_lib_module()
...
class NalaLlmUploadTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.mod = load_nala_llm_upload_module()
...
class RootAndLayerResolutionTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        loader = SourceFileLoader("nala_cli_mod", str(BIN / "helpers" / "nala_cli.py"))
        spec = importlib.util.spec_from_loader("nala_cli_mod", loader)
    ...
    def test_resolve_default_root_uses_project_dotdir_in_git_repo(self):
        ...
        self.assertEqual(root.resolve(), (project / ".nala").resolve())
    ...
    def test_resolve_default_root_falls_back_to_user_root(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                os.chdir(previous_cwd)
        self.assertEqual(root, home / ".nala")
    ...
    def test_resolve_prompt_path_layers_root_then_user_then_install(self):
        with tempfile.TemporaryDirectory() as tmp:
            home = Path(tmp) / "home"
            root = Path(tmp) / "root"
            (home / ".nala" / "prompts").mkdir(parents=True)
            ...
            with unittest.mock.patch.dict(os.environ, {"HOME": str(home)}):
                ...
                user_copy = home / ".nala" / "prompts" / name
    ...
    def test_tool_discovery_layers_shadow_by_basename(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            user_bin = Path(tmp) / "home" / ".nala" / "bin"
    ...
    def test_default_config_path_layers(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            os.chdir(project)
            try:
                env = {"HOME": str(home), "NALA_CONFIG": str(Path(tmp) / "env.json")}
                with unittest.mock.patch.dict(os.environ, env):
                    # NALA_CONFIG wins.
                    self.assertEqual(
                        nala_llm.default_config_path(), Path(tmp) / "env.json"
                    )
                with unittest.mock.patch.dict(os.environ, {"HOME": str(home)}, clear=False):
                    os.environ.pop("NALA_CONFIG", None)
                    # No project config yet: user config.
                    self.assertEqual(
                        nala_llm.default_config_path(),
                        home / ".nala" / "config.json",
                    )
                    # Project config exists: it wins over the user config.
                    project_config = project / ".nala" / "config.json"
                    project_config.parent.mkdir(parents=True)
                    project_config.write_text("{}", encoding="utf-8")
                    self.assertEqual(nala_llm.default_config_path(), project_config)
            finally:
                os.chdir(previous_cwd)
...
class ParseResponseTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.mod = load_nala_module()

    def test_valid_multi_tag_response(self):
        text = (
            '<nala-response>Hello</nala-response>\n'
            '<nala-read path="/tmp/foo" />\n'
            '<nala-file-read path="/tmp/big.py" />\n'
            '<nala-file-patch index="/tmp/split/index.json" />\n'
            "<nala-next>continue</nala-next>\n"
            "<nala-conversation>delegate</nala-conversation>\n"
            '<nala-conversation conversation-file="existing-conv">continue file</nala-conversation>\n'
            '<nala-conversation conversation-name="saved-conv">continue saved</nala-conversation>'
        )
    ...
    def test_conversation_tag_rejects_unsupported_options(self):
        tags, ignored, err = self.mod.parse_response(
            '<nala-conversation unknown="conv">delegate</nala-conversation>'
        )
        self.assertEqual(tags, [])
        self.assertIn("Unsupported <nala-conversation> attribute", err)

        tags, ignored, err = self.mod.parse_response(
            '<nala-conversation conversation-file="conv" conversation-name="saved">'
            "delegate</nala-conversation>"
        )
        self.assertEqual(tags, [])
    ...
    def test_write_tag_carries_raw_content(self):
        body = 'line1\nif a < b && c: print("&")\nline3\n'
        tags, ignored, err = self.mod.parse_response(
            f'<nala-write path="/tmp/out.py">{body}</nala-write>'
        )
        self.assertIsNone(err)
    ...
    def test_read_tag_requires_exactly_path_attribute(self):
        ...
        tags, ignored, err = self.mod.parse_response("<nala-read />")
        self.assertEqual(tags, [])
        self.assertIn('requires exactly one path="..."', err)

        tags, ignored, err = self.mod.parse_response('<nala-read path="/tmp/f" extra="x" />')
        self.assertEqual(tags, [])
        self.assertIn('requires exactly one path="..."', err)

    def test_shell_tag_rejects_attributes(self):
        tags, ignored, err = self.mod.parse_response('<nala-shell mode="x">ls</nala-shell>')
        self.assertEqual(tags, [])
    ...
    def test_leading_prose_is_ignored_not_rejected(self):
        tags, ignored, err = self.mod.parse_response(
            "oops <nala-response>Hi</nala-response>"
        )
        self.assertIsNone(err)
        self.assertEqual([t.kind for t in tags], ["response"])
    ...
    def test_trailing_prose_after_valid_tag_is_ignored(self):
        tags, ignored, err = self.mod.parse_response(
            "<nala-shell>find .</nala-shell> Standard input linter is used."
        )
        self.assertIsNone(err)
    ...
    def test_echoed_agent_response_wrapper_is_unwrapped(self):
        tags, ignored, err = self.mod.parse_response(
            "<agent-response>\n<nala-conversation>do it</nala-conversation>\n</agent-response>"
        )
        self.assertIsNone(err)
    ...
    def test_distinct_tags_are_not_deduped(self):
        ...
        tags, ignored, err = self.mod.parse_response(
            "<nala-next>a</nala-next><nala-next>b</nala-next>"
            '<nala-read path="/tmp/x" /><nala-read path="/tmp/y" />'
        )
        self.assertIsNone(err)
...
class ActionTests(unittest.TestCase):
    ...
    def test_execute_read_binary_file_returns_error_result(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            read_result = self.mod.execute_read(str(path))
            self.assertIn('<nala-read-result path="', read_result)
            self.assertIn("error=", read_result)

            file_read_result = self.mod.execute_file_read(str(path), Path(tmp))
            self.assertIn('<nala-file-read-result path="', file_read_result)
    ...
    def test_execute_shell(self):
        result = self.mod.execute_shell("echo hello-nala")
        self.assertIn("hello-nala", result)
    ...
    def test_execute_shell_handles_non_utf8_output(self):
        ...
        self.assertIn("<nala-shell-result>", result)
        self.assertIn("exit_code: 0", result)
        self.assertIn("\ufffd", result)

    def test_shell_output_precedes_next_input_in_either_order(self):
        # A turn with both <nala-shell> and <nala-next> must record the
        ...
        # only collects next-prompts; run_agent_loop appends <nala-next-input>
        # afterwards. That ordering must hold regardless of tag order, because
        # <nala-next> never appends in the loop.
        def conversation_after(turn):
            with tempfile.TemporaryDirectory() as tmp:
                ...
                _resp, next_prompts, _cont = self.mod.process_tags(
                    tags, conv, Path(tmp), None, NALA, "pid", self.mod.TokenStats()
                )
                # mirror run_agent_loop's post-process_tags next-input append
                for prompt in next_prompts:
                    self.mod.append_to_conversation(
                        conv, f"<nala-next-input>\n{prompt}\n</nala-next-input>"
                    )
                return conv.read_text(encoding="utf-8")

        for turn in (
            "<nala-shell>echo SHELL_MARKER</nala-shell><nala-next>go</nala-next>",
            "<nala-next>go</nala-next><nala-shell>echo SHELL_MARKER</nala-shell>",
        ):
            text = conversation_after(turn)
            i_shell = text.find("SHELL_MARKER")
            i_next = text.find("<nala-next-input>")
    ...
    def test_parse_llm_json_output_includes_tokens(self):
        parsed = self.mod.parse_llm_json_output(
            json.dumps(
                {
                    "response": "<nala-response>ok</nala-response>",
                    "input_tokens": 12,
                    "output_tokens": 3,
                }
            )
        )
        self.assertEqual(parsed, ("<nala-response>ok</nala-response>", 12, 3))
    ...
    def test_call_llm_updates_token_stats_from_json(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            fake_llm.write_text(
                ...
                "import json\n"
                "print(json.dumps({"
                "'response': '<nala-response>token ok</nala-response>', "
                "'input_tokens': 17, "
                "'output_tokens': 5"
                "}))\n",
                encoding="utf-8",
            ...
            self.mod.NALA_LLM_TEXT = fake_llm
        ...
        self.assertEqual(output, "<nala-response>token ok</nala-response>")
    ...
    def test_run_agent_loop_retries_llm_errors(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def fake_call_llm(*args, **kwargs):
                ...
                return "<nala-response>retry ok</nala-response>", None
    ...
    def test_run_agent_loop_reports_after_llm_retry_exhaustion(self):
        ...
        self.assertEqual(
            responses,
            ["Error: LLM provider failed after 3 attempts; nala cannot continue."],
        )
    ...
    def test_run_agent_loop_appends_initial_reads_before_prompt(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(
                self.mod,
                "call_llm",
                return_value=("<nala-response>ok</nala-response>", None),
            ):
                code, responses = self.mod.run_agent_loop(
                    conversation,
                    root,
    ...
    def test_hook_per_run_runs_before_every_turn(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            call_llm = unittest.mock.Mock(
                side_effect=[
                    ("<nala-shell>echo work</nala-shell>", None),
                    ("<nala-response>done</nala-response>", None),
                ]
            )
    ...
    def test_hook_per_file_edit_runs_after_file_patch(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            tags, _ignored, err = self.mod.parse_response(
                '<nala-file-patch index="/tmp/nonexistent-index.json" />'
            )
            self.assertIsNone(err)
    ...
    def test_execute_agent_passes_pid(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def fake_run(cmd, **kwargs):
                ...
                class Result:
                    returncode = 0
                    stdout = "<nala-response>done</nala-response>"
                    stderr = ""
                return Result()
    ...
    def test_summarize_conversation_sends_conversation_prompt_to_llm(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Summarize the following nala conversation", captured["prompt"])
            self.assertIn("conversation history", captured["prompt"])
            self.assertIn("Do not summarize nala as a tool", captured["prompt"])
...
class InitialTextTests(unittest.TestCase):
    ...
    def test_delegated_initial_text(self):
        ...
        self.assertIn("parent nala conversation spawned you", text)
        self.assertIn("spawn a sub-conversation only when it buys something", text)
        self.assertIn("<nala-conversation>{prompt}</nala-conversation>", text)
        self.assertNotIn("<nala-agent>", text)
    ...
    def test_project_context_included_from_git_toplevel(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            os.chdir(project / "sub")
            try:
                text = self.mod.build_initial_context(root, NALA.resolve(), "user", "conv")
            finally:
                os.chdir(previous_cwd)
    ...
    def test_context_layers_ordered_install_user_project_root(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            home = Path(tmp) / "home"
            (home / ".nala").mkdir(parents=True)
            (home / ".nala" / "context.md").write_text("USER-MARKER", encoding="utf-8")
            project = Path(tmp) / "proj"
            project.mkdir()
            subprocess.run(["git", "init"], cwd=project, check=True, capture_output=True)
            ...
            try:
                with unittest.mock.patch.dict(os.environ, {"HOME": str(home)}):
                    text = self.mod.build_initial_context(
                        root, install / "bin" / "nala", "user", "conv"
                    )
            finally:
                os.chdir(previous_cwd)
    ...
    def test_context_layers_dedup_when_root_is_user_root(self):
        # Outside a repo the root is ~/.nala: the user layer and the root
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                with unittest.mock.patch.dict(os.environ, {"HOME": str(home)}):
                    text = self.mod.build_initial_context(
                        home / ".nala", install / "bin" / "nala", "user", "conv"
                    )
            finally:
                os.chdir(previous_cwd)
    ...
    def test_project_context_not_duplicated_inside_install_checkout(self):
        # Running nala from within its own checkout: the project toplevel is
    ...
    def test_git_repo_context_in_repo(self):
        with unittest.mock.patch.object(self.mod.subprocess, "run") as mock_run:
            mock_run.side_effect = [
                unittest.mock.Mock(returncode=0, stdout="/home/macton/nala\n", stderr=""),
                unittest.mock.Mock(
                    returncode=0,
                    stdout="origin\tgit@github.com:macton/nala.git (fetch)\n"
                    "origin\tgit@github.com:macton/nala.git (push)\n",
                    stderr="",
                ),
            ]
            context = self.mod.git_repo_context(Path("/home/macton/nala"))
        self.assertIn("- git toplevel/project-root: /home/macton/nala", context)
        self.assertIn("git remote -v:", context)
        self.assertIn("origin\tgit@github.com:macton/nala.git (fetch)", context)
    ...
    def test_create_initial_text_in_nala_repo(self):
        repo_root = Path(__file__).resolve().parent.parent
        text = self.mod.create_initial_text(
            repo_root / ".nala",
            NALA.resolve(),
    ...
    def test_create_initial_text_includes_bin_tool_descriptions(self):
        ...
        self.assertIn("nala-file-split", text)
        self.assertIn("nala-file-patch", text)
...
class ToolDescriptionTests(unittest.TestCase):
    ...
    def test_nala_description_does_not_require_prompt(self):
        result = subprocess.run(
            [str(NALA), "--description"],
            capture_output=True,
            text=True,
        )
        self.assertEqual(result.returncode, 0, result.stderr)
...
class CliTests(unittest.TestCase):
    ...
    def test_list_providers_cli(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "nala-root"
            result = subprocess.run(
                [str(NALA), "--root", str(root), "--list-providers"],
                capture_output=True,
                text=True,
                env=self.clean_env(),
            )
    ...
    def test_status_shows_reasoning_level_name(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [str(NALA), "--root", str(root), "--conversation", "c", "--config", str(config), "--status"],
                capture_output=True,
                text=True,
                env=self.clean_env(),
            )
            ...
            override = subprocess.run(
                [str(NALA), "--root", str(root), "--conversation", "c", "--config", str(config),
                 "--reasoning", "1", "--status"],
                capture_output=True,
                text=True,
                env=self.clean_env(),
    ...
    def test_status_prints_path_size_provider_and_model(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(lines[1], "provider:openrouter model:deepseek/deepseek-v4.1-flash reasoning:default")
    ...
    def test_clear_archives_existing_conversation(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [str(NALA), "--root", str(root), "--conversation", conv, "--clear"],
                capture_output=True,
                text=True,
                env=self.clean_env(),
            )
    ...
    def test_default_root_is_project_dotdir_inside_git_repo(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [str(NALA), "--conversation", "conv", "--status"],
                capture_output=True,
                text=True,
                cwd=project,
                env=self.clean_env(),
    ...
    def test_explicit_root_still_wins_inside_git_repo(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [str(NALA), "--root", str(explicit), "--conversation", "conv", "--status"],
                capture_output=True,
                text=True,
                cwd=project,
                env=self.clean_env(),
    ...
    def test_nala_seeds_conversation(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            conv = "test-seed"
            fake_llm = root / "fake-llm"
            fake_llm.write_text(
                "#!/usr/bin/python3\nprint('<nala-response>seed ok</nala-response>')\n",
                encoding="utf-8",
            )
            fake_llm.chmod(0o755)

            mod = load_nala_module()
            mod.NALA_LLM_TEXT = fake_llm
            conversation_file = root / conv
            conversation_file.write_text(
                mod.create_initial_text(root, NALA.resolve(), "user", conv),
                encoding="utf-8",
            )
            ...
            self.assertIn("nala root:", contents)
            self.assertIn("nala process:", contents)
            self.assertIn("cwd:", contents)
            self.assertIn("host (uname -a):", contents)
            self.assertIn("Context management (every nala conversation", contents)
            self.assertIn("<nala-conversation>{prompt}</nala-conversation>", contents)
            self.assertIn("User invocation:", contents)
            self.assertIn("<user-prompt>", contents)
            self.assertIn("hello", contents)
            self.assertIn("<nala-response>seed ok</nala-response>", contents)


@unittest.skipUnless(os.environ.get("NALA_LIVE_TESTS") == "1", "set NALA_LIVE_TESTS=1 for live OpenRouter tests")
class LiveIntegrationTests(unittest.TestCase):
    def test_live_llm_text(self):
        ...
        try:
            result = subprocess.run(
                [str(NALA_LLM_TEXT), "--file", prompt_file, "--provider", "openrouter", "--model", "deepseek/deepseek-v4.1-flash"],
                capture_output=True,
                text=True,
                timeout=120,
            )
    ...
    def test_live_nala_simple_response(self):
        with tempfile.TemporaryDirectory() as tmp:
            result = subprocess.run(
                [
                    str(NALA),
                    ...
                    "Respond only with <nala-response>integration-ok</nala-response>",
                ],
                capture_output=True,
                text=True,
                timeout=180,
...
class NalaLlmConfigTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.mod = load_nala_llm_module()

    def test_fresh_install_defaults_to_openrouter_deepseek(self):
        with tempfile.TemporaryDirectory() as tmp:
            self.assertEqual(
                self.mod.resolve_settings(config_path=Path(tmp) / "missing.json"),
    ...
    def test_openrouter_streams_chat_completions_and_preserves_usage(self):
        ...
        with unittest.mock.patch.object(self.mod, "require_package", return_value=fake_openai), \
            ...
            result = self.mod.generate_text_with_usage("hello openrouter", "openrouter", "deepseek/deepseek-v4.1-flash")

        self.assertEqual(result.text, "openrouter ok")
        self.assertEqual(result.input_tokens, 11)
        self.assertEqual(result.output_tokens, 3)
        self.assertEqual(captured["model"], "deepseek/deepseek-v4.1-flash")
    ...
    def test_list_models_openrouter_parses_data_envelope(self):
        ...
        payload = json.dumps(
            {"data": [{"id": "deepseek/deepseek-v4.1-flash"}, {"id": "openai/gpt-5.1"}]}
        ).encode("utf-8")
        ...
        self.assertEqual(models, ["deepseek/deepseek-v4.1-flash", "openai/gpt-5.1"])
    ...
    def test_list_providers_catalog(self):
        ...
        self.assertEqual(by_name["openrouter"]["default_model"], "deepseek/deepseek-v4.1-flash")
    ...
    @unittest.mock.patch.dict(os.environ, {"TOGETHER_API_KEY": "test-key"})
    ...
    @unittest.mock.patch.dict(os.environ, {"OPENROUTER_API_KEY": "test-key"})
    def test_openrouter_upload_rejects_non_image(self):
        with tempfile.NamedTemporaryFile(suffix=".pdf") as handle:
            with self.assertRaises(ValueError):
                self.mod._openrouter_upload(Path(handle.name), "summarize", "deepseek/deepseek-v4.1-flash")
    ...
    def test_utilities_autodetect_config_without_flags(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.dict(
                os.environ,
                {"OPENAI_API_KEY": "test-key", "NALA_CONFIG": str(config_path)},
                clear=False,
            ):
