...
class ParseElementTests(unittest.TestCase):
    def test_content_element(self):
        node = parse_element("<nala-shell>echo hi</nala-shell>")
        self.assertEqual(node.name, "nala-shell")
        self.assertEqual(node.content, "echo hi")
        self.assertFalse(node.self_closing)
        self.assertEqual(node.attrs, {})
        self.assertEqual((node.start, node.end), (0, 32))

    def test_self_closing_with_attribute(self):
        node = parse_element('<nala-read path="/tmp/foo" />')
    ...
    def test_raw_content_keeps_unescaped_specials(self):
        ...
        node = parse_element(f"<nala-shell>{body}</nala-shell>")
    ...
    def test_content_spans_newlines(self):
        node = parse_element("<nala-write path=\"/tmp/f\">line1\nline2\n</nala-write>")
...
class UnwrapTests(unittest.TestCase):
    def test_unwraps_whole_output_wrapper(self):
        self.assertEqual(
            unwrap_whole_element("  <nala-response>Hi</nala-response>\n", "nala-response"),
            "Hi",
        )

    def test_refuses_partial_wrapper(self):
        text = "<nala-response>a</nala-response> trailing"
        self.assertIsNone(unwrap_whole_element(text, "nala-response"))
    ...
    def test_refuses_ambiguous_nested_close(self):
        ...
        text = "<nala-response>a</nala-response> mid <nala-response>b</nala-response>"
        self.assertIsNone(unwrap_whole_element(text, "nala-response"))

    def test_refuses_wrong_name_attrs_and_self_closing(self):
        self.assertIsNone(unwrap_whole_element("<other>x</other>", "nala-response"))
        self.assertIsNone(
            unwrap_whole_element('<nala-response a="1">x</nala-response>', "nala-response")
        )
        self.assertIsNone(unwrap_whole_element("<nala-response />", "nala-response"))
        self.assertIsNone(unwrap_whole_element("not a tag", "nala-response"))
